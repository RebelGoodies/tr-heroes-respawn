...
function Unlock_Hero_Options(hero_data)
	...
	if hero_data.vacant_hero_slots > 0 and (table.getn(hero_data.available_list) - hero_data.free_hero_slots) > 0 then
...
function Get_Active_Heroes(init, hero_data)
	...
	if hero_data.free_hero_slots > 0 then
		table.insert(text_list, "OPEN: "..tostring(hero_data.free_hero_slots))
	end
	local vacant_count = hero_data.total_slots - hero_data.free_hero_slots - admiral_count
	if vacant_count > 0 then
		table.insert(text_list, "VACANT (Dead): "..tostring(vacant_count))
	end
